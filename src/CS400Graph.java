...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {        
    	if(start == null || end == null)
    		throw new NoSuchElementException("Null Inputs");
    	if(vertices.get(start) == null || vertices.get(end) == null)
    		throw new NoSuchElementException("Non Existant Verticies");
    	...
    	while(!pq.isEmpty()) {
    		Path current = (CS400Graph<T>.Path) pq.remove();		
    		if (current.end.equals(vertices.get(end))) {
    			//System.out.println("break");
    			shortest = current;
    			break;
    		}
    		if(!checkNotVisited(visited, current)) 
    			continue;
    		shortest = current;
    		visited.add(current);
    		
    		for(int i = 0; i < current.end.edgesLeaving.size(); i++) {
    			//System.out.println("loop");
    			Path newPath = new Path(current);
    			newPath.extend(newPath.end.edgesLeaving.get(i));
    			if(checkNotVisited(visited, newPath)) {
    				//System.out.println("added");
    				pq.add(newPath);
    			}
    		}
    	}
    	if(!shortest.end.equals(vertices.get(end)))
    		throw new NoSuchElementException("Can't Reach End V");
    	//System.out.println(shortest.dataSequence);
    	return shortest;
    }
    
    /**
     * Private helper method that checks if vertex was visited.
     * @param visited
     * @param newPath
     * @return true when not visited false otherwise
     */
    private boolean checkNotVisited(LinkedList<Path> visited, Path newPath) {
    	for(int j = 0; j < visited.size(); j++) {
			if(visited.get(j).end.equals(newPath.end)) {
